...
async def download_and_send(app, chat_id, manga_info: dict, chapter: dict, status_msg=None, ftype: str = None, user_id: int = None):
    """
    Direct-download helper used by /dl and ddl_ callbacks.
    Fetches page images via the source scraper, builds PDF, sends to chat.
    """
    ...
    title = manga_info.get("title") or "manga"
    chap_title = chapter.get("title") or chapter.get("num") or "chapter"
    qitem = None
    if user_id:
        try:
            ...
            qitem = None
    src_name = manga_info.get("src")
    scraper = mgr.get(src_name) if src_name else None
    if not scraper:
        raise RuntimeError("Source scraper not available")

    chap_url = chapter.get("url") or chapter.get("link")

    if status_msg:
        try:
            await status_msg.edit_caption(
                f"<b>⬇️ Direct Download</b>\n\n"
                f"<blockquote><b>{title}</b>\nChapter: <code>{chap_title}</code></blockquote>\n"
                f"<i>Fetching page list…</i>"
            )
        except Exception:
            try:
                await status_msg.edit_text(
                    f"<b>⬇️ Direct Download</b>\n\n<blockquote><b>{title}</b>\nChapter: <code>{chap_title}</code></blockquote>\n<i>Fetching page list…</i>"
                )
            except Exception:
                pass

    # Get page URLs – different scrapers expose different method names
    pages = []
    for meth in ("get_chapter", "get_pages", "get_images", "fetch_pages"):
        fn = getattr(scraper, meth, None)
        if callable(fn):
            try:
                pages = await fn(chap_url) if chap_url else await fn(chapter)
                if pages:
                    break
            except TypeError:
                try:
                    pages = await fn(chapter)
                    if pages:
                        break
                except Exception:
                    pass
            except Exception as e:
                log.warning(f"[DDL] {meth} failed: {e}")

    if not pages:
        # some scrapers need manga+chapter objects
        try:
            if hasattr(scraper, "get_chapter_images"):
                pages = await scraper.get_chapter_images(manga_info, chapter)
        except Exception as e:
            log.warning(f"[DDL] get_chapter_images: {e}")

    if not pages:
        raise RuntimeError("Could not fetch chapter pages from source")

    if ftype and str(ftype).lower() in ("links", "link", "urls", "images"):
        lines = [f"{i+1}. {u}" for i, u in enumerate(pages[:80])]
        body = "\n".join(lines)
        if len(pages) > 80:
            body += f"\n… +{len(pages)-80} more"
        title = manga_info.get("title") or "manga"
        ...
        cap = (
            f"<b>🔗 {title}</b>\n"
            f"<blockquote>Chapter: <code>{chap_title}</code>\n"
            f"Pages: <code>{len(pages)}</code></blockquote>\n"
            f"<pre>{body[:3500]}</pre>"
        )
        await app.send_message(chat_id, cap, disable_web_page_preview=True)
        if status_msg:
            try:
    ...
    if status_msg:
        try:
            await status_msg.edit_text(
                f"<b>⬇️ Direct Download</b>\n\n"
                f"<blockquote><b>{title}</b>\nChapter: <code>{chap_title}</code>\n"
                f"Pages: <code>{len(pages)}</code></blockquote>\n"
                f"<i>Downloading images…</i>"
            )
        except Exception:
            pass

    work = Path(tempfile.mkdtemp(prefix="manhua_dl_"))
    try:
        async with DL() as dl:
            await dl.get_imgs(pages, work)
            pdf_path = await dl.make(work, title, str(chap_title), type="pdf")
            ...
            caption = (
                f"<b>📥 {title}</b>\n"
                f"<blockquote>Chapter: <code>{chap_title}</code>\n"
                f"Source: <code>{src_name}</code></blockquote>"
            )
            ...
            if qitem:
                try:
                    await dl_queue.set_status(qitem.id, "done")
                except Exception:
                    pass
            return pdf_path
    finally:
...
def apply_pdf_password(pdf_path, password: str):
    """Password-protect PDF in place (pypdf). Returns path or original on failure."""
    if not password:
        return pdf_path
    try:
        ...
        out = path.with_name(path.stem + "_locked.pdf")
        ...
        import logging
        logging.getLogger(__name__).warning(f"pdf password failed: {e}")
        return pdf_path
