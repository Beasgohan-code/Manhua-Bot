...
**The complete [yt-dlp](https://github.com/yt-dlp/yt-dlp) engine, bundled in this repository, plus a `pydlp` command.**

Every extractor (1,750+ sites), every downloader, every option — unchanged upstream yt-dlp, under your own command name.

```
pip install git+https://github.com/Beasgohan-code/Py-dlp.git

pydlp "https://www.youtube.com/watch?v=BaW_jenozKc"
pydlp -F URL && pydlp -f "bv*+ba/b" URL
```

- `pydlp` — the Py-dlp CLI (branded usage/version, engine-update hints)
- `yt-dlp` — the bundled engine, also usable under its own name

The `yt_dlp/` directory is **byte-identical to upstream**; only the small
`pydlp/` package is added. Update the engine by replacing `yt_dlp/` from
